...
function _sensible_fluxs(net, flx_idx::Int, biom_idx::Int, biom0, biom1; 
        atol = 1e-3, npoints = 5, solver = Gurobi.Optimizer, delta = 1e-1
    )
...
function _force_nut_limited(lep, glc_id, biom_id, exchs_ids::Vector; 
    ...
    # max biom
    opm = FBAOpModel(lep, solver)
    set_linear_obj!(opm, biom_idx, 1.0)
    optimize!(opm)
    ...
    for it in 1:niters
        ...
        @time try
            ...
            sen_idxs = setdiff(sen_idxs, exchs_idxs, [biom_idx], protect_idxs, traj_idxs)
            ...
            @show ko_eps
            @show l0, u0
            bounds!(lep, rand_idx, -ko_eps, ko_eps)

...
let
    ...
    _, h0 = withcachedat(PROJ, :get!, cid) do
        ...
        _th_readdir(n1, n0; nthrs = 1) do bbi, bb
            ...
            _th_fealens, _th_feabioms = Float64[], Float64[]
...
## ------------------------------------------------------------
## ------------------------------------------------------------
# # DONE: cambia todo po histogramas2d
# # DONE: do histogram_rage(dim1, dim2, ...)
# # nut shadow price vs downset length histogram
# let
#     exchs = [
...
#     n0 = 0 # init file
#     n1 = 20 # non-ignored file count
#     for (exchi, exch) in enumerate(
#         [
#             # "EX_glc__D_e", 
#             "EX_lac__D_e", 
#             # "EX_malt_e",
...
#             _fealens, _obj_ms = Float64[], Float64[]
#             _th_readdir(n1, n0; nthrs = 8) do bbi, bb
...
#                         price = feasets_blob1[mkey]
#                         abs(price) < 1e-3 && continue # ignore non exch limited
#                         push!(_th_fealens, _fealen)
#                         push!(_th_obj_ms, feasets_blob1[mkey])
#                     end
#                 end

#                 # reduce
#                 lock(lk) do
#                     push!(_fealens, _th_fealens...)
#                     push!(_obj_ms, _th_obj_ms...)
#                 end

#                 return :continue
#             end # _th_readdir
#             return _fealens, _obj_ms
#         end
#         fealens, obj_ms = ret
        
#         # Plots
#         # uidxs = eachindex(fealens)
#         # uidxs = uidxs[1:100:end]
#         # @show length(uidxs)
#         # return uidxs

#         # ax = Axis(f[reverse(fldmod1(exchi, 2))...]; 
#         #     title = exch,
#         #     xlabel = "downset length", ylabel = "shadow price",
#         # )
#         # scatter!(ax, fealens[uidxs], obj_ms[uidxs]; label = exch)
#         return Plots.histogram2d(fealens, obj_ms; bins = 80, 
#             title = exch,
#             xlabel = "downset length", ylabel = "shadow price",
#         )
#     end
#     # f[1, 3] = Legend(f, ax, "Exchange", framevisible = false)
#     f
# end
